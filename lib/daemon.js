...
function DaemonInterface(daemons, options, logger){
// function DaemonInterface(options, logger){

   //  var daemons = options.daemons;
    //private members
    var _this = this;
    ...
    console.error("    options.coin.rpcapiv1 === true ");
              callback(allOnline);
              if (!allOnline)
    ...
    console.error("    options.coin.rpcapiv1 === false ");
              callback(allOnline);
              if (!allOnline)
                _this.emit('connectionFailed', results);
           });
