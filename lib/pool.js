...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var apifn = '';
        if ( options.coin.rpcapiv1 === "true") {
        apifn = 'getblockchaininfo';
        } else {
        apifn = 'getinfo';
        }

            // _this.daemon.cmd('getblockchaininfo', [], function(results) {
            _this.daemon.cmd( apifn , [], function(results) {
    ...
    function SubmitBlock(blockHex, callback){
        ...
        if (options.hasSubmitMethod){
            ...
            rpcArgs = [blockHex];
console.dir("Submit ->" + rpcCommand + rpcArgs);
        }
        else{
        ...
        _this.daemon.cmd(rpcCommand,
            rpcArgs,
            function(results){
        console.log("submitblock - results");
        console.dir(results);
            ...
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx){
                        ...
                        emitLog('DEBUG Accepted ' + String(tx));
                        console.log("check sharedata.blockHash:" + shareData.blockHash.toString());
                ...
                emitLog('DEBUG SUBMIT BLOCKHEX ' + String(blockHex));
                emitLog('DEBUG SUBMIT BLOCK ' + String(shareData.blockHash.toString()));
            }
    ...
    function SetupDaemonInterface(finishedCallback){
        ...
        _this.daemon = new daemon.interface(options.daemons, options, function(severity, message){
        // _this.daemon = new daemon.interface(options, function(severity, message){
            _this.emit('log', severity , message);
        });
    ...
    if (options.coin.rpcapiv1 === "true") {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            if (options.coin.rpcapiv1 === "true") {
                ...
                options.testnet = (rpcResults.getinfo.chain === 'test') ? true : false;
